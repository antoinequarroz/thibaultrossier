...
export default async function handler(req, res) {
  if (req.method === 'POST') {
    ...
    if (!fullName || !email || !address || !items) {
      return res
        .status(400)
        .json({ message: 'Données de commande manquantes.' });
    }
    ...
    const itemsList = items
      .map(
        (item) =>
          `- Produit ID: ${item.productId}, Taille: ${item.size}, Quantité: ${item.quantity}`
      )
      .join('\n');
      ...
      html: `
                ...
                <ul>
                    ${items
                      .map(
                        (item) =>
                          `<li>Produit ID: ${item.productId}, Taille: ${item.size}, Quantité: ${item.quantity}</li>`
                      )
                      .join('')}
                </ul>
            `,
    ...
    try {
      ...
      return res
        .status(500)
        .json({ message: 'Erreur lors de la soumission de la commande.' });
    }
  } else {
